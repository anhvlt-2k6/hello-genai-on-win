...
def test_static_files():
    ...
    process = subprocess.Popen(
        ...
        env=dict(os.environ, PORT="8080", DEBUG="false")
    )
    ...
    try:
        ...
        # Test the favicon
        response = requests.get("http://localhost:8080/static/favicon.ico")
        assert response.status_code == 200, f"Failed to get favicon.ico: {response.status_code}"
        print("✅ Favicon test passed")
        
        # Test robots.txt
        response = requests.get("http://localhost:8080/static/robots.txt")
